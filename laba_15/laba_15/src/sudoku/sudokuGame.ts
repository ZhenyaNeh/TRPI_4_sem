import { boxSize, findEmptyCell, generateSudoku, gridSize, NestedArray, removeCells } from "./generateSudokuGrid";

export type DublicateCords = Array<{row: number, column: number}>

export class Sudoku {
    grid: NestedArray;
    answerGrid: NestedArray;

    constructor(){
        this.answerGrid = generateSudoku();
        this.grid = removeCells(this.answerGrid);
    }
